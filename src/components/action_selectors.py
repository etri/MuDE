...
class EpsilonGreedyActionSelector():
    ...
    def select_action(self, agent_inputs, agent_inputs_plus, avail_actions, t_env, t_ep, test_mode=False):
        ...
        if self.args.whole_seq == 1:
            if t_ep == 0:
                self.rand_one = th.rand_like(agent_inputs[0,0,0])
        else:
            self.rand_one = th.rand_like(agent_inputs[0,0,0])

        criNum = (1-self.args.Decay_Ratio_Start)/self.args.Act_Decay_End * t_env + self.args.Decay_Ratio_Start

        if criNum > 1:
            criNum = 1
            
        if self.rand_one <= criNum:
            masked_q_values = agent_inputs.clone()
        else:
            masked_q_values = agent_inputs_plus.clone()
